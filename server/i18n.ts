...
const translations: Record<"en" | "es", Translations> = {
  en: {
    ...
    // Notes
    noteRateLimit: (maxRequests, window) =>
      `Limit: ${maxRequests} downloads per ${window}.`,
    noteCleanup: (ttl) =>
      `Each file is automatically deleted after download or ${ttl}.`,
  ...
  es: {
    ...
    // Notes
    noteRateLimit: (maxRequests, window) =>
      `Límite: ${maxRequests} descargas por ${window}.`,
    noteCleanup: (ttl) =>
      `Cada archivo se elimina automáticamente tras la descarga o ${ttl}.`,
